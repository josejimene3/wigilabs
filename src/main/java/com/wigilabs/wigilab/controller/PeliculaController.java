    ...
    public ResponseEntity<?> listarId(@PathVariable(value = "id") Long id) {
        ...
        if (id > 0) {
            ...
            if (pelicula == null) {
                ...
                response.put("response", "La película no existe");
                return new ResponseEntity<>(response, HttpStatus.OK);
            }
        } else {
            response = new HashMap<>();
            response.put("error", "1");
            response.put("response", "El id de la película no existe");
            return new ResponseEntity<>(response, HttpStatus.OK);
        }
        response = new HashMap<>();
        response.put("error", "0");
    ...
    public ResponseEntity<?> eliminar(@PathVariable(value = "id") Long id) {
        ...
        if (id > 0) {
            ...
            if (pelicula == null) {
                response = new HashMap<>();
                response.put("error", "1");
                response.put("mensaje", "La película no existe");
                return new ResponseEntity<>(response, HttpStatus.OK);
            }
        } else {
            response = new HashMap<>();
            response.put("error", "1");
            response.put("mensaje", "El id de la película no existe");
            return new ResponseEntity<>(response, HttpStatus.OK);
        }
